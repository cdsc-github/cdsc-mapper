...
$ECHO_CMD "\033[36m[PolyOpt] Compile PolyOpt/HLS $@\033[0m";
echo "=> execute: PolyHLS  -rose:C99 $CDSCMP_SDSL_C_POLYOPT_USER_OPTIONS $@ ";
PolyHLS $@ -rose:C99 $CDSCMP_SDSL_C_POLYOPT_USER_OPTIONS;
if [ $? -ne 0 ]; then
    $ECHO_CMD "\033[36m[PolyOpt] Error with Compile PolyOpt/HLS $@\033[0m";
    exit 1;
fi;
